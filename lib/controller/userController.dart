...
//     var uniqueKey = firebaseRef.collection("users");
    ...
    Future<UploadTask?> uploadFilesForOrder(XFile? files,context,String name,String phoneNumber,String email,String gender,String accessary,String measure,String message) async {
...
//     var uniqueKey = firebaseRef.collection("users");
  ...
  // });
    ...
    DocumentReference refs = await FirebaseFirestore.instance.collection('orders').add(newOrder);
    await FirebaseFirestore.instance.collection('orders').doc(refs.id).update({
    "docid":refs.id
   });
...
await getPlayerId();
   ...
   await FirebaseFirestore.instance.collection('users').doc(useri).update({
    "playerId":playerId
   });
